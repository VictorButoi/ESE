...
def viz_accuracy_vs_confidence(
    ...
    if isinstance(pixel_preds, pd.core.frame.DataFrame):
        ...
        for bin_num, x_var_set in data_dict.items():
            ...
            avg_bin_met_meters = {measure_group: StatsMeter() for measure_group in measure_dict.keys()}
        ...
        total_weighted = sum(bin_samples["weighted"].values()) # Take int because weighting causes small residual error.
        ...
        for bin_num, x_var_set in avg_data_dict.items():
            for x_var in x_var_set.keys():
                ...
                avg_data_dict[bin_num][x_var]["proportion"] = {}
                avg_data_dict[bin_num][x_var]["proportion"]["mean"] = bin_x_samples / bin_samples["uniform"][bin_num]
                ...
                avg_data_dict[bin_num][x_var]["weighted proportion"] = {}
                avg_data_dict[bin_num][x_var]["weighted proportion"]["mean"] = weighted_x_samples / bin_samples["weighted"][bin_num]

            # Finally, normalize by the total number of samples.
            avg_data_dict[bin_num]["avg"]["proportion"] = {}
            avg_data_dict[bin_num]["avg"]["proportion"]["mean"] = bin_samples["uniform"][bin_num] / total_uniform 
            # Finally, normalize by the weighted number of samples.
            avg_data_dict[bin_num]["avg"]["weighted proportion"] = {}
            avg_data_dict[bin_num]["avg"]["weighted proportion"]["mean"] = bin_samples["weighted"][bin_num] / total_weighted 
